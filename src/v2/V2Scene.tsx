...
function Toggle({ checked, onChange, label }: { checked: boolean; onChange: (v: boolean) => void; label: string }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
      <span style={{ color: '#8090b0', fontSize: 13 }}>{label}</span>
      <div
...
function HUD({ cSim, setCSim, showGrid, setShowGrid, onBack, containerRef }: {
  ...
  return (
    <div style={{ fontFamily:"'SF Mono','Fira Code',monospace", position: 'fixed', inset: 0, pointerEvents: 'none', zIndex: 10 }}>
      <ScenePanel
        title="V2 — Light Cone Engine"
        onBack={onBack}
        containerRef={containerRef}
        theme="blue"
        filename="v2-light-cone.webm"
      >
        {/* ── c_sim slider ──────────────────────────────── */}
        <div style={{ marginBottom: 16 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, fontSize: 13 }}>
            <span style={{ color: '#b0bcde' }}>c<sub>sim</sub> — vitesse lumière</span>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: '#3a4a60', marginTop: 3 }}>
            <span>lent — grand halo</span>
            <span>rapide — halo nul</span>
          </div>
        </div>

        <Toggle label="Afficher la grille" checked={showGrid} onChange={setShowGrid} />

        {/* ── Légende ───────────────────────────────────── */}
        <div style={{ padding: '10px 12px', background: 'rgba(0,0,0,0.35)', borderRadius: 7, marginBottom: 12 }}>
          <div style={{ fontSize: 11, color: '#3a4a60', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 8 }}>Légende</div>
          {[
            { icon: '●', color: '#aabbff', text: 'Position perçue (retardée)' },
            { icon: '○', color: '#6680cc', text: 'Position réelle (fantôme)' },
            { icon: '·', color: '#8899cc', text: 'Traînée — chemin de la lumière' },
          ].map(({ icon, color, text }) => (
            <div key={text} style={{ display: 'flex', gap: 8, marginBottom: 6, alignItems: 'flex-start', fontSize: 13, color: '#7080a0' }}>
              <span style={{ color, flexShrink: 0, fontSize: 15, lineHeight: 1.2 }}>{icon}</span>
              <span>{text}</span>
            </div>
          ))}
        </div>
        ...
        <div style={{ padding: '9px 11px', background: 'rgba(107,140,255,0.06)', borderRadius: 7, fontSize: 12, color: '#4a5888', lineHeight: 2.0 }}>
          ...
          Halo ∝ <span style={{ color: '#ff8c6b' }}>1 / c<sub>sim</sub></span> — bisection 24 iter<br />
          <span style={{ color: '#2a3a60' }}>Drag orbite · Scroll zoom</span>
        </div>
      </ScenePanel>
    </div>
  )
...
export function V2Scene({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div ref={containerRef} style={{ width: '100vw', height: '100vh', background: '#04060f', position: 'relative' }}>
      <Canvas
